...
gwr.multiscale <- function(formula, data, kernel="bisquare", adaptive=FALSE, criterion="dCVR", max.iterations=2000,threshold=0.00001, dMats, p.vals, theta.vals,longlat=FALSE,
                     bws0, bw.seled=rep(F, length(bws0)), approach = "AIC", bws.thresholds=rep(0.1, length(dMats)), bws.reOpts=5,verbose=F,hatmatrix=T, nlower = 10)
{
  ...
  dMat <- dMats[[1]]
  bw.int0 <- bw.gwr1(x, y, dp.locat,approach=approach,kernel=kernel,adaptive=adaptive,dMat,verbose=F, nlower = nlower)
  ...
  res <- gwr.q2(x, y, dp.locat, adaptive=adaptive, hatmatrix = hatmatrix,bw=bw.int0, kernel=kernel,dMat=dMat)
  betas <- res[[1]]
  if(hatmatrix)
  {
    Shat <- res[[2]]
  ...
  { 
     ...
     for(i in 1:var.n)
     {
       ...
       bws0[i] <- bw.i       
       res <- gwr.q2(matrix(x[,i], ncol=1), y.i, dp.locat, adaptive=adaptive, hatmatrix = hatmatrix,bw=bw.i, kernel=kernel,dMat=dMat)
       betai <- res[[1]]
       if(hatmatrix)
       {
         Si <- res[[2]]
...
#     resid.i <- y - gw.fitted(x, betas) 
#   }
   #Q<-t(diag(dp.n)-Shat)%*%(diag(dp.n)-Shat) 
   #RSS.gw<-t(y)%*%Q%*%y 
   RSS.gw <- RSS1
